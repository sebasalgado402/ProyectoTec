<?php
session_start();
   if(isset($_SESSION['usuarioActivo']) && strlen($_SESSION['usuarioActivo'])>0){
   ...
   if(isset($_POST['input__Usuario']) && isset($_POST['input__Password'])){
        if(strlen($_POST['input__Usuario']) >0 && strlen($_POST['input__Password'])>0){
            if(!empty($_POST['input__Usuario']) || !empty($_POST['input__Password'])){  
                ...
                $consulta = "SELECT usu_nombre , usu_contraseña FROM usuarios where usu_nombre='".$_POST['input__Usuario']."' and usu_contraseña = '".$_POST['input__Password']."'";
